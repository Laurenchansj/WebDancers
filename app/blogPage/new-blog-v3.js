...
export default function NewBlogV3() {
  ...
  const handleDayDescription = (dayLocations) => {
    const descriptions = [];
    for (let i = 0; i < duration; i++) {
      const singleDayDes = dayLocations[i].description;
      descriptions.push(singleDayDes[0]);
    }
    setDayDescription((singleDayDes) => [...dayDescription, singleDayDes]);
  };
    ...
    if (duration === 0 || title.length === 0 || description.length === 0) {
      ...
      try {
        const daysData = Array.from({ length: duration }, (_, i) => {
          ...
          // console.log("one location" + dayLocations[0].description); // will get the first day locations
          // for (let i = 0; i < duration; i++) {
          //   const singleDayDes = dayLocations[i].description;
          //   setDayDescription((singleDayDes) => [
          //     singleDayDes,
          //     ...dayDescription,
          //   ]);
          // }

          handleDayDescription(dayLocations);
      ...
      <div style={{ display: "flex", flexDirection: "row" }}>
        ...
        >
          ...
          >
            ...
            <div
              className="p-5 border border-cyan-600 rounded-lg bg-sky-50 w-fit"
              // style={{ width: "400px" }}
            >
              <table>
                <tbody>
        ...
        <Blog blog={blog} dayDescription={dayDescription} />
        {/* <Blog blog={blog} /> */}
      </div>
    </div>
  );
